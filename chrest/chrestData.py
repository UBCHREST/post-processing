...
class ChrestData:
    ...
    def compute_mean_rms_file(self, field, ablateorexp):
        
        
        output_file = field + "_stats_" + ablateorexp + ".h5"
        new_folder_path = self.files[0].parent / 'data'
        if not new_folder_path.exists():
            new_folder_path.mkdir()
            
        outfoldfile = new_folder_path / output_file
        hdf5_dest = h5py.File(outfoldfile, 'w')
        initiated = False
        count=0
        
        for hdf5_file in self.files:
            if (hdf5_file.suffix == ".hdf5" and hdf5_file!=output_file) or (hdf5_file.suffix == ".h5" and hdf5_file!=output_file):
                ...
                source_field = hdf5_source["data/fields/" + field]
                # set up the output on the first time
                if not initiated:
                    initiated = True
                    
                    data = hdf5_dest.create_group('data')
                    # store the metadata
                    for key in self.metadata:
                        data.attrs.create(key, self.metadata[key])
                    data.attrs.create('time', self.times[0])
                    ...
                    fields = data.create_group('fields')
                    for field in self.new_data:
                        newField = fields.create_dataset(field, data=self.new_data[field][0], compression="gzip",
                                                         dtype=np.float32)
                        if field in self.new_data_component_names:
                            newField.attrs.create('components', self.new_data_component_names[field])
                    ...
                    # fname = hdf5_file.name
                    # b = fname.replace('.h5', '')
                    # field_avg = hdf5_dest.create_dataset(b +"_avg", source_field.shape, 'f', fillvalue=0)
                    # field_avg.attrs.update(source_field.attrs)
                    # field_rms = hdf5_dest.create_dataset(b + "_rms", source_field.shape, 'f',
                    #                                       fillvalue=0)
                    # field_Re = hdf5_dest.create_dataset(b +"_baruv", (102,102,400), 'f', fillvalue=0)
                    # field_Re.attrs.update(source_field.attrs)
                    # field_rms.attrs.update(source_field.attrs)
                    
                    fname = hdf5_file.name
                    b = fname.replace('.h5', '')
                    field_avg = fields.create_dataset("avg", source_field.shape, 'f', fillvalue=0)
                    field_avg.attrs.update(source_field.attrs)
                    
                    field_rms = fields.create_dataset("rms", source_field.shape, 'f',
                                                          fillvalue=0)
                    field_rms.attrs.update(source_field.attrs)
                    if field == "vel":
                        field_Re = fields.create_dataset("baruv", source_field.shape[0:3], 'f', fillvalue=0)
                        field_Re.attrs.update(source_field.attrs)
                    
                    
                    
                field_avg[:] = np.add(field_avg[:], source_field[:])
                field_rms[:] = np.add(field_rms[:], np.square(source_field[:]))
                if field == "vel":
                    field_Re[:] = np.add(field_Re[:],(source_field[:,:,:,0])*(source_field[:,:,:,1]))
                hdf5_source.close()
            
                # print("Added in contribution from ", hdf5_file)
        # Now finish
        field_avg[:] = field_avg[:] / count
        field_rms[:] = field_rms[:] / count - np.square(field_avg[:])
        field_rms[:] = np.maximum(field_rms[:], 0)
        field_rms[:] = np.sqrt(field_rms[:])
        if field == "vel":
            field_Re[:] = field_Re[:] / count
